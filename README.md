...
- 🚀 **无需API密钥** - 使用原生 `fetch` 调用YouTube前端公开的内部API
- 🎯 **全量获取** - 不受页面加载限制，自动获取所有评论
- 📊 **实时进度** - 按钮显示获取进度："⏳ 已获取 123 条..."
- ⚙️ **可配置上限** - `maxCommentsToFetch` 默认1000条，可自定义
- 🔄 **智能降级** - API模式关闭时自动降级为DOM模式（仅复制可见评论）
- 💪 **包含回复** - 完整获取所有主评论和回复
- 📍 **集成在评论区** - 按钮直接显示在评论数旁边，符合使用直觉

**按钮位置：**
```
评论区顶部:  [123 条评论] [📋 复制所有评论]
                            ↑ 直接点击，无需配置
```
...
1. 打开任意有评论的YouTube视频
2. 滚动到评论区
3. 点击评论数旁边的 "📋 复制所有评论" 按钮
4. 等待自动获取（实时显示进度）
5. 完成后自动复制到剪贴板

**配置项：**
```javascript
fetchAllComments: true,        // 启用API模式（推荐）
maxCommentsToFetch: 1000,      // 最大获取数量
```

**技术细节：**
- ✅ 使用原生 `fetch` API，不需要 GM 权限
- ✅ 调用 `youtubei/v1/next` 公开端点（YouTube前端自己用的）
- ✅ 使用 continuation token 机制分页获取
- ✅ 每批请求间隔500ms避免限流
- ✅ 自动解析评论和回复的完整数据
- ✅ 完全免费，无配额限制
...
**使用方法：**
1. 打开任意有评论的视频，滚动到评论区
2. 评论数旁边会出现 "📋 复制所有评论" 按钮
3. 点击按钮，脚本会自动通过API获取所有评论（**无需API密钥**）
4. 实时显示进度："⏳ 已获取 123 条..."
5. 完成后自动复制到剪贴板
6. 格式：`序号. @用户名:\n评论内容\n\n`

**两种模式：**
- **API模式**（`fetchAllComments: true`，推荐）- 使用YouTube内部API，自动获取所有评论，无需滚动页面
- **DOM模式**（`fetchAllComments: false`）- 只复制页面上已加载的可见评论

**为什么不需要API密钥？**
脚本运行在YouTube页面上下文中，使用的是YouTube前端自己使用的公开API端点（`youtubei/v1/next`），通过浏览器的原生 `fetch` 发送请求，无需额外配置。
